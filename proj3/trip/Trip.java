...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        Road nextRoad;
        double nextWeight = 0;
        for (int i = 1; i < segment.size() - 1; i += 1) {
            Road r = _map.getLabel(segment.get(i - 1), segment.get(i));
            double weight = r.length();
            Direction direction = r.direction();
            String name = r.toString();
            nextRoad = _map.getLabel(segment.get(i), segment.get(i + 1));
            continueName = nextRoad.toString();
            continueDir = nextRoad.direction();
            nextWeight = nextRoad.length();
            while (direction.equals(continueDir) && name.equals(continueName)) {
                i += 1;
                weight += nextWeight;
                if (i != segment.size() - 1) {
                    nextRoad = _map.getLabel(segment.get(i),
                            segment.get(i + 1));
                    continueName = nextRoad.toString();
                    continueDir = nextRoad.direction();
                    nextWeight = nextRoad.length();
            ...
            if (i != segment.size() - 1) {
                tripPrint(seq, name, direction, weight);
                seq += 1;
            } else {
                break;
            }
        }
        int index = segment.size() - 1;
        String destinationName = _map.getLabel(segment.get(index)).toString();
        lastPrint(seq, continueName, continueDir, nextWeight, destinationName);
        seq += 1;
        return seq;
    }

    /** Print the route we take.
     * @param seq sequence
     * @param name name
     * @param dir direction
     * @param weight weight*/
    private void tripPrint(int seq, String name, Direction dir, double weight) {
        System.out.printf("%d. Take %s %s for %.1f miles.", seq, name,
                dir.fullName(), weight);
        System.out.println();
    }

    /** Print the road to the destination.
     * @param continueName name of next road
     * @param continueDir next directon
     * @param nextWeight next weight
     * @param destinationName name of destination
     * @param seq the order of a road */
    private void lastPrint(int seq, String continueName, Direction continueDir,
                           double nextWeight, String destinationName) {
        System.out.printf("%d. Take %s %s for %.1f miles to %s.",
                seq, continueName, continueDir.fullName(),
                nextWeight, destinationName);
        System.out.println();
    }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double getWeight(int u, int v) {
            double result = _map.getLabel(u, v).length();
            return result;
        }

        @Override
        protected double estimatedDistance(int v) {
            double result = _map.getLabel(v).distance();
            return result;
        }
