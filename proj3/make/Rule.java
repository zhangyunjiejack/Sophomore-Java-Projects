...
class Rule {
    ...
    private void checkFinishedDependencies() {
        Iteration<Integer> temp = _depends.successors(_vertex);
        while (temp.hasNext()) {
            int curVertex = temp.next();
            if ((_depends.getLabel(curVertex)).isUnfinished()) {
                String text = _target
                        + (_depends.getLabel(curVertex)).getTarget();
                Main.error("Error error" + text);
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        while (successors.hasNext()) {
            int curVertex = successors.next();
            if (_time < _depends.getLabel(curVertex).getTime()) {
                return true;
            }
        }
        return false;
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            if (_commands.isEmpty()) {
                error(" Error error %s needs to be rebuilt"
                                + ", but has no commands",
                      _target);
            }
